...
class TestErrorPattern:
    ...
    def test_error_pattern_regex_compilation(self):
        ...
        assert pattern.pattern.pattern == "simple pattern"
...
class TestBasePatternMatcher:
    """Test base pattern matcher functionality"""
    
    def test_extract_context_functionality(self):
        """Test context extraction from matched errors"""
        class TestMatcher(BasePatternMatcher):
...
line 4"""
        ...
        # Should return 3 lines of context around the match
        assert context is not None
        assert "line 1" in context or "line 2" in context
        assert "line 2 with error here" in context
        assert "line 3" in context
    
    def test_extract_context_no_match(self):
        """Test context extraction when match text not found in lines"""
        class TestMatcher(BasePatternMatcher):
            def get_patterns(self) -> List[ErrorPattern]:
                return []
        ...
        # Create a mock match that won't be found in the text lines
        import re
        from unittest.mock import Mock
        
        mock_match = Mock()
        mock_match.group.return_value = "nonexistent_text"
...
class TestPythonPatternMatcher:
    ...
    def test_key_error_pattern_matching(self, python_matcher):
        """Test KeyError pattern recognition"""
        patterns = python_matcher.get_patterns()
        
        # Find KeyError pattern
        ...
        if key_error_pattern:
            # Test pattern matches typical KeyError messages
            test_messages = [
                "KeyError: 'missing_key'",
                'KeyError: "another_key"',
                "KeyError: 'special-key-123'"
            ]
            
            for message in test_messages:
                match = key_error_pattern.pattern.search(message)
                assert match is not None, f"Pattern should match: {message}"
...
class TestJavaScriptPatternMatcher:
    ...
    def test_javascript_language_indicators(self, js_matcher):
        ...
        # Test that indicators can identify JavaScript errors
        js_error_text = "ReferenceError: variable is not defined"
        
        # At least one indicator should match JavaScript-style errors
        matches_found = False
        for pattern in indicators:
            if pattern.search(js_error_text):
                matches_found = True
                break
        
        # Note: This might not always match depending on implementation
...
class TestJavaPatternMatcher:
    """Test Java-specific pattern matching"""
    
    @pytest.fixture
    def java_matcher(self):
        """Create Java pattern matcher for testing"""
    ...
    def test_java_language_indicators(self, java_matcher):
        ...
        assert len(indicators) >= 0  # May be empty in minimal implementation
        
        # If indicators exist, test basic functionality
        for pattern in indicators:
            assert isinstance(pattern, re.Pattern)
    
    def test_java_error_patterns(self, java_matcher):
        """Test Java error patterns"""
        patterns = java_matcher.get_patterns()
        
        # Should have at least some patterns or be empty for minimal implementation
        assert len(patterns) >= 0
        
        # If patterns exist, they should be properly configured
        for pattern in patterns:
            assert "java" in pattern.languages
            assert isinstance(pattern.severity, ErrorSeverity)
            assert isinstance(pattern.category, ErrorCategory)
...
class TestErrorPatternMatcher:
    ...
    def test_pattern_recognizer_initialization(self, recognizer):
...
IndexError: list index out of range"""
    ...
    def test_detect_language_with_no_indicators(self, recognizer):
        """Test language detection with ambiguous text"""
        generic_text = "This is just some generic text without language indicators"
        
        detected_language = recognizer.detect_language(generic_text)
        
        # Should return None for text with no language indicators
        assert detected_language is None or isinstance(detected_language, str)
    ...
    def test_find_matches_with_language_filter(self, recognizer):
        """Test finding matches with language filtering"""
        error_text = "IndexError: list index out of range"
        
        # Search with Python language filter
        python_matches = recognizer.find_matches(error_text, language="python")
        
        assert isinstance(python_matches, list)
        
        # All matches should be for Python language
        for match in python_matches:
            assert "python" in match.pattern.languages
    
    def test_find_matches_sorting(self, recognizer):
        """Test that matches are sorted by severity and confidence"""
        # This test assumes we have multiple patterns that could match
        error_text = "Error: multiple potential matches"
        
        matches = recognizer.find_matches(error_text)
        
        if len(matches) > 1:
            # Check that matches are sorted by severity (critical first)
            severity_order = ["critical", "high", "medium", "low", "info"]
            
            for i in range(len(matches) - 1):
                current_severity_idx = severity_order.index(matches[i].pattern.severity.value)
                next_severity_idx = severity_order.index(matches[i + 1].pattern.severity.value)
                
                # Current should be same or higher priority (lower index) than next
                assert current_severity_idx <= next_severity_idx
    ...
    def test_get_best_match_with_language(self, recognizer):
        """Test getting best match with language specification"""
        error_text = "IndexError: list index out of range"
        
        best_match = recognizer.get_best_match(error_text, language="python")
        
        if best_match:
            assert isinstance(best_match, ErrorMatch)
            assert "python" in best_match.pattern.languages
    
    def test_get_best_match_no_matches(self, recognizer):
        """Test getting best match when no patterns match"""
        # Use text that shouldn't match any patterns
        non_error_text = "This is just regular text with no error patterns"
        
        best_match = recognizer.get_best_match(non_error_text)
...
IndexError: list index out of range"""
    ...
    def test_multiple_error_patterns_in_text(self):
        """Test recognition of multiple error patterns in single text"""
        recognizer = ErrorPatternMatcher()
        
        multi_error_text = """
        First error: IndexError: list index out of range
        Second error: KeyError: 'missing_key' 
        Third error: TypeError: unsupported operand
        """
        
        matches = recognizer.find_matches(multi_error_text)
        
        # Should find multiple matches if patterns exist
        if len(matches) > 1:
            # Matches should be for different error types
            error_names = [match.pattern.name for match in matches]
